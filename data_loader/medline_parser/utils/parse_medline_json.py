    ...
    for item_index in range(len(dict)):
        ...
        if len(dict[item_index]["authors"]) != 0:
            for author_index in range(len(dict[item_index]["authors"])):
                ...
                temp_dict["authors"][author_index]["name"] = dict[item_index]["authors"][author_index]["forename"] + " " + dict[item_index]["authors"][author_index]["lastname"]
                temp_dict["authors"][author_index]["affiliation"] = dict[item_index]["authors"][author_index]["affiliation"] = dict[item_index]["authors"][author_index]["affiliation"]
        else:
            continue
        if dict[item_index]["abstract"] is not None and dict[item_index]["abstract"] != "":
